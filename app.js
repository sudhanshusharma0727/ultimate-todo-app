/* ===================================================
   Ultimate Todo App — Application Engine (Firebase)
   =================================================== */
import { subscribeToAuthChanges, initUser, subscribeToTodos, subscribeToUserData, addTodo as dbAddTodo, updateTodo as dbUpdateTodo, removeTodo as dbDeleteTodo, updateUserData, logout } from './firebase-config.js';
...
subscribeToAuthChanges(async (currentUser) => {
    ...
    unsubscribeTodos = subscribeToTodos(user.uid, (newTodos) => {
        todos = newTodos;
        render();
    });
    ...
    unsubscribeUser = subscribeToUserData(user.uid, (data) => {
        if (data) {
            ...
            if (data.settings) {
                ...
                if (data.settings.collapsed) collapsedState = data.settings.collapsed;
            }
            renderProjects();
            renderTags();
            render();
    ...
    cb.addEventListener('change', () => {
        ...
        for (const id of todoIds) {
            ...
            if (todo) {
                ...
                dbUpdateTodo(user.uid, todo);
            }
        }
        // render handled by snapshot
    });
    return cb;
}
...
function toggleComplete(id) {
    ...
    // Copy so we don't mutate state directly before sync
    ...
    if (todo.completed && todo.recurring) {
        ...
        dbAddTodo(user.uid, n);
    }

    dbUpdateTodo(user.uid, updatedTodo);
    checkConfetti();
}
...
function deleteTodo(id) {
    ...
    dbDeleteTodo(user.uid, id);
    pushUndo('Task deleted', todo);
}
...
function renderSubtasks(todo) {
    ...
    for (let i = 0; i < todo.subtasks.length; i++) {
        ...
        cb.addEventListener('change', () => {
            ...
            txtInput.classList.toggle('subtask-item__text--done', cb.checked);
            dbUpdateTodo(user.uid, { ...todo });
        });
        ...
        txtInput.addEventListener('blur', () => {
            st.text = txtInput.value.trim() || st.text;
            dbUpdateTodo(user.uid, { ...todo });
        });
...
function updateBadges() {
    ...
    // renderProjects(); // No need to re-render buttons, just badges? 
    // Actually renderProjects rebuilds the buttons and badges, so it works.
    // Optimization: separate render from badge update only? For now keep it simple.
    renderProjects();
    renderTags();
}
...
//  POMODORO
// ============================================================
// Simplified pomodoro (no sync for sessions count for now, keep local or move to settings later)
